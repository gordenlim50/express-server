...
def xyz(light,mode,n):
#  xyz(light,1,1) -->tristimulus for light source
#  xyz(light,1,2) -->chromaticity for light source
#  xyz(light,2,1) -->tristimulus for reflected light from Munsell color
#  xyz(light,2,2) -->chromaticity for reflected light from Munsell color
...
## CIE 1931 2-degrees Color Matching Function
    ...
    y_bar = np.expand_dims(y_bar, axis=1)
    z_bar = np.expand_dims(z_bar, axis=1)
    
#    print('x_bar.shape', x_bar.shape)
    
    if (mode==1):
        # Tristimulus (XYZ) of Light Source in test
        X = np.dot(light,x_bar)
        Y = np.dot(light,y_bar)
        Z = np.dot(light,z_bar)
       
#         print('X.shape', X.shape)
#         print('Y.shape', Y.shape)
#         print('Z.shape', Z.shape)
        
        # Chromaticity (xyz) coordinate of Light Source in test 
        x1=X/(X + Y + Z)
        y1=Y/(X + Y + Z)
        z1=Z/(X + Y + Z)
